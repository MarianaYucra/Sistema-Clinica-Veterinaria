...
def test_registrar_cliente_invalidos_pe(cliente_entorno, id_cli, nombre, motivo):
    """Partición de Equivalencia (PE): Valida que datos vacíos lancen un ValueError."""
...
def test_registrar_cliente_duplicado_avl(cliente_entorno):
    """Análisis de Valores Límite (AVL): Controla la frontera de IDs únicos en el sistema."""
    _, svc = cliente_entorno
...
def test_buscar_cliente_existente_y_no_existente(cliente_entorno):
    """Valida la lógica de búsqueda para fronteras de éxito y error."""
    _, svc = cliente_entorno
    svc.registrar("123", "Juan", "987", "j@a.com")
...
def test_listar_clientes(cliente_entorno):
    """Verifica que el reporte y listado de clientes en memoria funcione de forma aislada."""
    _, svc = cliente_entorno
